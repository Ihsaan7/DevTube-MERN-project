...
const RegisterPage = () => {
  ...
  return (
    <div className={`min-h-screen flex transition-colors duration-300 ${isDark ? 'bg-neutral-950' : 'bg-white'}`}>
      {/* Left Side - Branding */}
      <div className={`hidden lg:flex lg:w-2/5 relative overflow-hidden ${isDark ? 'bg-neutral-900' : 'bg-neutral-50'}`}>
        ...
        <div className="relative z-10 flex flex-col justify-between p-16">
          ...
          <div className="space-y-6">
            <div className="flex items-start gap-4 group">
              ...
              <div>
                <h3 className={`font-semibold mb-1 ${isDark ? 'text-white' : 'text-neutral-900'}`}>Quick Setup</h3>
                <p className={`text-sm ${isDark ? 'text-neutral-500' : 'text-neutral-600'}`}>Get started in minutes with our simple registration</p>
              </div>
            </div>
            <div className="flex items-start gap-4 group">
              <div className={`w-1 h-12 ${isDark ? 'bg-white' : 'bg-neutral-900'} transition-all duration-300 group-hover:h-16`} />
              <div>
                <h3 className={`font-semibold mb-1 ${isDark ? 'text-white' : 'text-neutral-900'}`}>Free Forever</h3>
                <p className={`text-sm ${isDark ? 'text-neutral-500' : 'text-neutral-600'}`}>No hidden fees or premium tiers required</p>
              </div>
            </div>
            <div className="flex items-start gap-4 group">
              <div className={`w-1 h-12 ${isDark ? 'bg-white' : 'bg-neutral-900'} transition-all duration-300 group-hover:h-16`} />
              <div>
                <h3 className={`font-semibold mb-1 ${isDark ? 'text-white' : 'text-neutral-900'}`}>Global Reach</h3>
                <p className={`text-sm ${isDark ? 'text-neutral-500' : 'text-neutral-600'}`}>Share your videos with audiences worldwide</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12 relative">
        <div className="w-full max-w-2xl">
          ...
          <div className={`border p-8 transition-all duration-300 ${
            ...
            <div className="mb-6">
              <h2 className={`text-3xl font-bold mb-2 ${isDark ? 'text-white' : 'text-neutral-900'}`}>Create account</h2>
              <p className={isDark ? 'text-neutral-400' : 'text-neutral-600'}>Join StreamVault and start sharing</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {/* Avatar Upload */}
                <div className="group">
                  <label htmlFor="avatar" className={`block text-sm font-medium mb-2 transition-colors ${
                    isDark ? 'text-neutral-300 group-focus-within:text-white' : 'text-neutral-700 group-focus-within:text-neutral-900'
                  }`}>
                    Avatar * <span className={`text-xs ${isDark ? 'text-neutral-500' : 'text-neutral-500'}`}>(Profile)</span>
                  </label>
                  <input
                    type="file"
                    id="avatar"
                  ...
                  {avatarPreview && (
                    <div className="mt-2">
                      <img
                        src={avatarPreview}
                        alt="Avatar"
                        className={`w-12 h-12 object-cover border ${isDark ? 'border-neutral-700' : 'border-neutral-300'}`}
                      />
                    </div>
                  )}
                </div>
                ...
                <div className="group">
                  <label htmlFor="coverImage" className={`block text-sm font-medium mb-2 transition-colors ${
                    isDark ? 'text-neutral-300 group-focus-within:text-white' : 'text-neutral-700 group-focus-within:text-neutral-900'
                  }`}>
                    Cover <span className={`text-xs ${isDark ? 'text-neutral-500' : 'text-neutral-500'}`}>(Optional)</span>
                  </label>
                  <input
                    type="file"
                    id="coverImage"
                  ...
                  {coverImagePreview && (
                    <div className="mt-2">
                      <img
                        ...
                        alt="Cover"
                        className={`w-full h-12 object-cover border ${isDark ? 'border-neutral-700' : 'border-neutral-300'}`}
                      />
                    </div>
                  )}
                </div>
